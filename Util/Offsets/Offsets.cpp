...
#define GETPATT(x, mod, patt) x = U::Pattern.Find(mod, patt);

void CUtil_Offsets::Initialize()
{
	GETPATT(m_dwClientModeCreateMove, "client.dll", "55 8B EC FF 75 ? F3 0F 10 45 ? 51 F3 0F 11 04 24 E8");
	GETPATT(m_dwIInput_GetUserCmd, "client.dll", "55 8B EC 53 56 8B D9 8B 4D 08 57 E8 ? ? ? ? 8B 75 0C 8B F8 B8 ? ? ? ?");
	GETPATT(m_dwIInput, "client.dll", "8B 0D ? ? ? ? 8B 01 8B 40 ? FF D0");
	...
	GETPATT(m_dwCTFGameMovement_ProcessMovement, "client.dll", "55 8B EC 53 8B 5D ? 57 8B F9 85 DB 0F 84");
	...
	GETPATT(m_dwCNetChan_Shutdown, "engine.dll", "57 8B F9 68 ? ? ? ? C7 07 ? ? ? ? E8 ? ? ? ? 8D 8F");
